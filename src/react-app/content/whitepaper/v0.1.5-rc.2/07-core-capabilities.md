...
### rc.2 的 Message Feedback

逐消息 Feedback 的权威状态由 `feedback/message-put` 与 `feedback/message-delete` 归约得到，评分为 `positive` / `negative`，可以附带分类和备注，并通过 opaque version 做乐观并发控制。Feedback 事件只进入日志，**不会进入模型上下文**。

rc.2 的 Web 交互把好评和差评统一为“先打开反馈弹窗、确认后提交”；提交失败时保留弹窗与草稿并给出提示。再次点击已经记录的评分则直接撤回。这个变化属于 Feedback Consumer / UI 语义，没有改变 Agent Loop 或 Session 的主链路。
...
官方包地图把大多数组视为产品稳定 API；`experimental` 不属于默认稳定能力，`e2b` 在 rc.2 中仍是 POC，`test-support`、`runtime-diagnostics` 与 `util` 的兼容性预期也更低。
