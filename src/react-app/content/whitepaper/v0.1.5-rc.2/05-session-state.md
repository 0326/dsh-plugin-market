...
当前版本的持久化写路径由生命周期持有的 `SessionHandle` 管理。创建或恢复持久 Session 时，Agent Loop 先取得对应写所有权，再发布可运行 Agent；同一持久 Session 同时至多被一个进程持有写权限。
...
`v0.1.5-rc.2` 使用 Session V3。官方迁移器会从受支持旧日志生成新版日志并保留原文件；迁移后的 Session 不支持降级读取。自定义日志读取器、导出器或分析工具需要按 V3 事件与 Surface 语义适配。
