...
const COPY = {
  zh: {
    kicker: "网站LOGO / 故事",
    tagline: "从深海游向 AI 世界的爱鲲",
    intro:
      "爱鲲（iKun），它从 DeepSeek 的深海意象出发，游进 AI 世界，代表的是对技术的无限热爱和开源精神。",
    ...
    brandText:
      "作为网站的吉祥物，爱鲲的特长是唱跳、代码和打篮球。唱跳代表对艺术的向往，代码代表对技术的追求，篮球则代表对运动和生活的热爱，这充分说明爱鲲是一名德智体美劳全面发展的练习生。选择爱鲲做网站代言人，寄托了我们对各位开发者的美好祝福。",
    browse: "浏览插件",
    docs: "查看文档",
    communityTitle: "社区驱动，开放协作",
    communityText:
      "DSH-PLUGIN MARKET 并非 DeepSeek 官方产品，欢迎通过 GitHub 参与改进。",
    source: "查看项目源码",
  },
  en: {
    kicker: "ABOUT / BRAND",
    ...
    intro:
      "The Kun is more than the DSH-PLUGIN MARKET logo. It starts with DeepSeek's deep-sea imagery, swims into the AI world, and carries a playful iKun signature from the developer community.",
    ...
    brandText:
      "Sing & dance, code, and basketball are playful motifs around the iKun brand and carry over the visual easter eggs from the homepage. They are not the iKun equation. The only brand equation is the origin chain above: DeepSeek + AI → Whale + AI → AI + Kun → Love + Kun → iKun.",
    browse: "Browse plugins",
    docs: "Read the docs",
    communityTitle: "Community-driven and open",
    communityText:
      "DSH-PLUGIN MARKET is not an official DeepSeek product. Contributions are welcome on GitHub.",
  ...
  return (
    <section className="about-brand-page">
      <section className="about-ocean-hero">
        <div
          className="about-ocean-wave about-ocean-wave-one"
          aria-hidden="true"
        />
        <div
          className="about-ocean-wave about-ocean-wave-two"
          aria-hidden="true"
        />
        ...
        <div className="about-brand-copy">
          ...
          <div className="about-brand-actions">
            <a className="btn about-btn-primary" href="/plugins">
              {copy.browse} →
            </a>
            <a
        ...
        <div className="about-kun-stage" aria-label={copy.tagline}>
          ...
          <span
            className="about-bubble about-bubble-three"
            aria-hidden="true"
          />
        ...
        <div className="about-equation-panel">
          ...
          <div
            className="about-equation-track"
            aria-label={copy.equation.join(" equals ")}
          >
            {copy.equation.map((item, index) => (
              <span
                className={`about-equation-term about-equation-term-${index + 1}`}
                key={item}
              >
                <strong>{item}</strong>
                {index < copy.equation.length - 1 && (
      ...
      <section className="about-hobby-section">
        ...
        <div className="about-hobby-grid">
          {copy.hobbies.map((hobby, index) => (
            <article
              className={`about-hobby-card about-hobby-${hobby.key}`}
              key={hobby.key}
            >
              ...
              <div className="about-hobby-icon" aria-hidden="true">
                ...
                {hobby.key === "code" && "</>"}
                {hobby.key === "ball" && (
                  <Icon name="ball-basketball" size={50} stroke={2.2} />
                )}
              </div>
      ...
      <section className="about-community-strip">
        ...
        <a
          ...
          href="https://github.com/0326/dsh-plugin-market"
          target="_blank"
          rel="noreferrer"
        >
